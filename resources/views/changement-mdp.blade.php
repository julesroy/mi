...
<body class="bg-white pt-28 md:pt-45 min-h-screen flex flex-col">
    ...
    <main class="flex-1 flex flex-col items-center justify-center">
        <form id="resetForm" method="POST" action="./"
            class="rounded-2xl border border-black p-5 [&_input]:font-sans flex items-center flex-col">
            @if ($errors->has('token'))
                @error('token')
            ...
            @else
                ...
                <input type="submit" value="Confirmer"
                    class="cursor-pointer bg-primaire text-white px-3 py-2 rounded-full border-1 border-black mt-5" />
            @endif
