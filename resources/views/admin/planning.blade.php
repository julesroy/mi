    ...
    <body class="pt-28 md:pt-54 h-screen">
        ...
        <dialog id="deleteDialog" class="backdrop-brightness-50 backdrop-blur-xs flex items-center justify-center w-full bg-transparent h-full absolute top-0 d not-open:hidden z-10 [&_button]:hover:cursor-pointer text-white">
            <div class="rounded-lg bg-gray-800 p-6 shadow-lg m-8">
                ...
                <div class="flex items-start justify-between">
                    <h2 class="text-xl font-bold text-gray-900 sm:text-2xl dark:text-white">Désinscription</h2>

                    <button type="button" class="-me-4 -mt-4 rounded-full p-2 text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-600 focus:outline-none dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300" onclick="document.getElementById('deleteDialog').close()" aria-label="Close">
                ...
                <div class="mt-4">
                    <p class="text-pretty text-gray-700 dark:text-gray-200">
                        Voulez-vous vraiment désinscrire
                        <b id="deleteDialog__name">Test</b>
                        du poste
                        <b id="deleteDialog__job">Poste</b>
                        le
                ...
                <footer class="mt-6 flex justify-end gap-2">
                    <button type="button" onclick="document.getElementById('deleteDialog').close()" class="rounded bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">Annuler</button>

                    <button type="button" onclick="deleteInscription()" class="rounded bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700">Désinscrire</button>
                </footer>
            </div>
        </dialog>
        ...
        <dialog id="inscriptionDialog" class="backdrop-brightness-50 backdrop-blur-xs flex items-center justify-center w-full bg-transparent h-full absolute top-0 d not-open:hidden z-10 [&_button]:hover:cursor-pointer text-white">
            <form id="inscriptionDialog__form" class="rounded-lg bg-gray-800 p-6 shadow-lg m-8" method="dialog">
                <input type="hidden" value="0" id="deleteDialog__id" />
                <div class="flex items-start justify-between">
                    <h2 class="text-xl font-bold text-gray-900 sm:text-2xl dark:text-white">Inscription</h2>

                    <button type="button" class="-me-4 -mt-4 rounded-full p-2 text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-600 focus:outline-none dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300" onclick="document.getElementById('inscriptionDialog').close()" aria-label="Close">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                ...
                <div class="flex flex-col text-gray-200 mt-4 [&_label]:text-lg [&>select]:mb-1.5 [&>select]:bg-gray-900 [&>select]:rounded-full [&>select]:py-2 [&>select]:px-4 [&>select]:cursor-pointer [&>select]:hover:bg-gray-700 [&>select]:transition-colors">
                ...
                <footer class="mt-6 flex justify-end gap-2">
                    <button type="button" onclick="document.getElementById('inscriptionDialog').close()" class="rounded bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">Annuler</button>

                    <button type="submit" class="rounded bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700">Inscire</button>
                </footer>
            </form>
        </dialog>
        ...
        <div id="planningWrapper" class="flex flex-col md:flex-row justify-center items-center gap-8 min-h-[90%] [&>*]:bg-gray-800 [&>*]:border-gray-700 [&>*]:border [&>*]:rounded-2xl day text-white">
            <div id="calendar" class="flex flex-col items-center">
                ...
                <span id="displayMode" class="flex flex-row justify-center gap-5 mb-2.5 [&_button]:text-gray-500">
                    <button onclick="setDisplayMode(true)" id="displayMode__day" class="font-bold hover:text-gray-300 cursor-pointer">Jour</button>
                    <button onclick="setDisplayMode(false)" id="displayMode__week" class="font-bold hover:text-gray-300 cursor-pointer">Semaine</button>
                </span>
                <div class="border-t border-t-gray-700 px-5 py-2.5">
                    <table>
                        ...
                        <tbody id="calendar__inner"></tbody>
                    </table>
                </div>
            </div>
                ...
                @foreach ([["blue", "Bar"], ["red", "Cuisine"], ["yellow", "Transit"], ["green", "Courses"]] as $index => $job)
                    <section class="p-5 pb-3 border-t-gray-700 border-t">
                        <h3 class="flex flex-row gap-2 text-2xl font-bold relative">
                            ...
                            {{ $job[1] }}
                            <button type="button" class="absolute right-1 cursor-pointer rounded-full text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-600 focus:outline-none dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300" onclick="showInscriptionDialog({{ $index }})" aria-label="Inscription">
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 12L18 12M12 6L12 18" />
                                </svg>
                            </button>
                        </h3>
                        <div id="day__{{ $index }}" class="flex flex-col m-1"></div>
                    </section>
                @endforeach
            </div>
            ...
            <div id="week" class="flex flex-col w-sm md:w-md lg:w-xl">
                @foreach ($weekDays as $index => $day)
                    <section class="p-5 pb-3 border-t-gray-700 {{ $index == 0 ? "" : "border-t" }}">
                        <h3 class="flex flex-row gap-2 text-2xl font-bold relative">
                            <button class="cursor-pointer hover:text-blue-500" id="week__name__{{ $index }}">
                                {{ $day }} -
                                <span id="week__date__{{ $index }}"></span>
                            </button>
                            <button type="button" id="week__inscription__{{ $index }}" class="absolute right-1 cursor-pointer rounded-full text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-600 focus:outline-none dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300" aria-label="Inscription">
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 12L18 12M12 6L12 18" />
                                </svg>
                            </button>
                        ...
                        <div class="flex flex-col gap-2.5 m-1">
                            ...
                            @for ($i = 0; $i < 4; $i++)
                                <div id="week__{{ $index }}__{{ $i }}" class="week-job-{{ $i }} flex flex-row flex-wrap gap-1.5 empty:hidden"></div>
                            @endfor
                        </div>
                    </section>
                @endforeach
        ...
        <script>
            ...
            function selectDay(day) {
                ...
                document.getElementById('day__name').innerHTML = WEEKDAYS[currentDate.getDay() - 1] + ' - ' + currentDate.getDate().toString().padStart(2, '0') + '/' + (currentDate.getMonth() + 1).toString().padStart(2, '0');
                ...
                for (let i in WEEKDAYS) {
                    ...
                    document.getElementById('week__date__' + i).innerHTML = weekDate.getDate().toString().padStart(2, '0') + '/' + (weekDate.getMonth() + 1).toString().padStart(2, '0');
            ...
            function setDisplayMode(day) {
                if (!day) document.getElementById('planningWrapper').classList.remove('day');
                else document.getElementById('planningWrapper').classList.add('day');
            }
            ...
            function deleteInscription() {
                ...
                fetch('planning/supprimer-inscription/' + document.getElementById('deleteDialog__id').value + '/' + dateSQLFormat(currentDate), {
                    headers: {
            ...
            function fetchCalendar() {
                ...
                fetch(window.location.pathname + '/data/' + dateString)
                    .then((r) => r.json())
                    .then(updateCalendar);
            }
            ...
            function updateCalendar(data) {
                ...
                for (let day in data) {
                    ...
                    let jobs = '';
                    for (let job in reservations) jobs += `<span class="text-xs px-1.5 py-0.5 rounded-full job-${job + (reservations[job].length == 0 ? ' opacity-30' : '')}">${reservations[job].length}</span>`;
