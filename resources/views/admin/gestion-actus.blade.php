        ...
        <div class="max-w-4xl mx-auto px-4 mb-10">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between rounded bg-gray-700 gap-4 mb-6 py-3 px-4">
                <form method="GET" action="{{ route('gestion-actus') }}" class="flex-1">
                    <input type="text" name="search" placeholder="Rechercher une actu..."
                        class="w-full md:w-72 px-3 py-2 rounded text-white bg-gray-900" value="{{ request('search') }}">
                </form>
                <button
                    id="openAddDialog"
                    type="button"
                    class="bg-blue-500 hover:bg-blue-700 text-white px-4 py-2 rounded font-semibold transition"
                >
                    + Ajouter une actu
                </button>
            </div>
                            ...
                            <tr id="row-{{ $actu->idActu }}">
                                ...
                                <td class="py-2 px-4 text-center align-middle" data-field="actions">
                                    <button type="button"
                                        ...
                                        onclick="editRow({{ $actu->idActu }})"
                                    >Modifier</button>
                                    <form method="POST"
                                        action="{{ route('actus.destroy', $actu->idActu) }}"
                                        class="inline-block"
                                        onsubmit="return confirm('Voulez-vous vraiment supprimer cette actu ?')"
                                        style="display:inline"
                                    >
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="rounded px-3 p-1 bg-gray-600 text-red-500 hover:underline"
                                        >Supprimer</button>
                                    </form>
                                </td>
                            </tr>
