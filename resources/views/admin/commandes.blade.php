...
<body class="bg-white text-black pt-28 md:pt-60">
    ...
    <main class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-wrap gap-2 mb-6 justify-center">
            <button onclick="filterCommandes('all')"
                class="px-4 py-2 bg-gray-400 hover:bg-gray-500 rounded-lg transition-colors">Toutes</button>
            <button onclick="filterCommandes(0)"
                class="px-4 py-2 bg-red-500 hover:bg-red-600 rounded-lg transition-colors">Non payées</button>
            <button onclick="filterCommandes(1)"
                class="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg transition-colors">Payées</button>
            <button onclick="filterCommandes(2)"
                class="px-4 py-2 bg-orange-500 hover:bg-orange-600 rounded-lg transition-colors">Prêtes</button>
            <button onclick="filterCommandes(3)"
                class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg transition-colors">Servies</button>
            <button onclick="filterCommandes(4)"
                class="px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg transition-colors">Annulées</button>
        </div>
    ...
    <dialog id="modifierCommandeDialog"
        class="bg-[#00000045] z-50 flex justify-center items-center fixed top-0 left-0 w-full h-full not-open:hidden">
        <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div class="p-6">
                <div class="flex justify-between items-center mb-4">
                    <span id="numeroCommandeModifiee" class="font-bold text-xl">N° commande</span>
                    <button onclick="document.getElementById('modifierCommandeDialog').close()"
                ...
                <div class="mb-4">
                    ...
                    <textarea id="commandeCommentaire" rows="3"
    ...
    <script>
        ...
        window.modifierCommande = function(id) {
            ...
            document.getElementById('numeroCommandeModifiee').innerText = `Commande n°${id}`;
            document.getElementById('modifierCommandeDialog').show();
    ...
    `;
            Promise.all([
                ...
                fetch('/admin/ingredients/all').then((r) => {
                    if (!r.ok) throw new Error(`Erreur inventaire: ${r.status}`);
                    return r.json();
                }),
            ]).then(([commandeResponse, inventaireResponse]) => {
            ...
            try {
                ...
                const response = await fetch(`/admin/commandes/modifier/${currentCommandeId}`, {
                    ...
                    body: JSON.stringify({
                        ...
                        items: currentCommandeItems,
                    }),
                });

                if (response.ok) {
                    alert('Modifications enregistrées avec succès');
                    document.getElementById('modifierCommandeDialog').close();
                    loadCommandes();
                } else {
