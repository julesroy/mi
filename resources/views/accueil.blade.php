            ...
            <section class="space-y-6">
                ...
                <div x-data="{
                    ...
                    <div class="absolute inset-0 flex justify-between items-center px-4">
                        <!-- Flèche gauche -->
                        <button @click="activeSlide = (activeSlide - 1 + totalSlides) % totalSlides" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-full">&#8592;</button>

                        <!-- Flèche droite -->
                        <button @click="activeSlide = (activeSlide + 1) % totalSlides" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-full">&#8594;</button>
                    </div>

                    <!-- Navigation Buttons -->
                    <div class="flex justify-center mt-4 space-x-4">
                        <button @click="activeSlide = 0" :class="{ 'bg-green-500': activeSlide === 0 }" class="w-3 h-3 rounded-full bg-gray-300 transition-colors duration-300"></button>
                        <button @click="activeSlide = 1" :class="{ 'bg-green-500': activeSlide === 1 }" class="w-3 h-3 rounded-full bg-gray-300 transition-colors duration-300"></button>
                        <button @click="activeSlide = 2" :class="{ 'bg-green-500': activeSlide === 2 }" class="w-3 h-3 rounded-full bg-gray-300 transition-colors duration-300"></button>
                    </div>
                </div>
                ...
                <div>@auth
                    ...
                    >
                        ...
                        <!-- Flèches de navigation -->
                        <div
                            class="absolute inset-0 flex justify-between items-center px-4"
                        >
                            <!-- Flèche gauche -->
                            <button
                    ...
                    @if (isset($actus) && $actus->isNotEmpty())
                        @foreach ($actus as $actu)
                            ...
                            <div
                                class="mb-1 text-sm"
                            >
                                {{ \Carbon\Carbon::parse($actu->date)->format("d/m/Y H:i") }}
                            </div>
                            <div
                                class="bg-white border rounded px-4 py-2 shadow mb-2"
                            >
                                {{ $actu->contenu }}
                            </div>
                        @endforeach
                ...
                <div>
                    @auth
                        <div class="bg-white border rounded-lg shadow p-4">
                            ...
                            <div>
                                Chaud :
                                <span class="font-semibold">5ème</span>
                            </div>
                            <div>
                                Froid :
                                <span class="font-semibold">aucun</span>
                            </div>
                        </div>
                    @endauth
